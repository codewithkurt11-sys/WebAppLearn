 ...
 *  2. displayName is read from storage (not inline in render).
 *  3. All expensive derived computations are memoised.
 *  4. shimmer animation defined in index.css — inline <style> tag removed.
 *  5. TrackCard / ContinueCard are memoized so the grid doesn't re-render
 *     when an unrelated state changes.
 *  6. SVG gradient has a unique ID to avoid conflicts when multiple Dashboards
 *     render (edge case) — now keyed per component instance.
 */
...
// Track IDs that have quiz / completion tracking (excludes "cheatsheet").
const TRACKED_IDS = TRACK_DEFS.filter(t => t.id !== "cheatsheet").map(t => t.id);
...
export default function Dashboard() {
  ...
  const gradientId = useId().replace(":", "").replace(":", ""); // safe for SVG ID

  const [recentPages, setRecentPages] = useState<string[]>([]);

  useEffect(() => {
    setRecentPages(storage.getRecentPages());
  }, []);

  // Derived stats — memoised.
  const stats = useMemo(() => {
    const completedCount = TRACKED_IDS.filter(id => progress[id]?.completed).length;
    const quizzesTaken   = TRACKED_IDS.filter(id => progress[id]?.score != null).length;
    const pct = TRACKED_IDS.length
      ? Math.round((completedCount / TRACKED_IDS.length) * 100)
      : 0;
    const scored = TRACKED_IDS
      .map(id => progress[id]?.score)
      .filter((s): s is number => s != null);
    const avgScore = scored.length
      ? Math.round(scored.reduce((a, b) => a + b, 0) / scored.length)
      : 0;
    return { completedCount, quizzesTaken, pct, avgScore };
  }, [progress]);

  const displayName = storage.getDisplayName() || user?.email?.split("@")[0] || "friend";

  const recentContinue = useMemo(() =>
    recentPages
  ...
  return (
    <div>
        ...
        <div style={{
          ...
          <div style={{ position: "relative", flex: 1, minWidth: 0 }}>
            ...
            <div style={{
              ...
              fontSize: 12, color: T.muted2, marginBottom: 18, marginTop: 8,
              letterSpacing: "0.3px",
            }}>// keep building</div>

            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <StatPill label="Completed" value={stats.completedCount} color={T.accent} loading={loading} />
              <StatPill label="Quizzes"   value={stats.quizzesTaken}   color={T.green}  loading={loading} />
              <StatPill label="Avg score" value={`${stats.avgScore}%`} color={T.amber}  loading={loading} />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <CircleProgress pct={stats.pct} size={isMobile ? 72 : 96} gradientId={gradientId} />
          )}
        </div>
      </div>

      {/* ─── Continue Learning ─── */}
      <section aria-label="Continue learning" style={{ padding: `0 ${pad} 24px` }}>
        <SectionDivider label="Continue learning" />
        {recentContinue.length === 0 ? (
          <div style={{
            padding: "16px 20px",
            background: T.surface, border: `1px solid ${T.border}`,
            borderRadius: 12, fontSize: 12.5, color: T.muted2,
            display: "flex", alignItems: "center", justifyContent: "space-between",
          }}>
            <span>You haven't started any lessons yet.</span>
            <button
              onClick={() => setPage("py-basics")}
              style={{
                background: "transparent", border: `1px solid ${T.border2}`,
                borderRadius: 8, color: T.accent, fontSize: 12,
      ...
      {/* ─── All Tracks ─── */}
      <section aria-label="All learning tracks" style={{ padding: `0 ${pad} 32px` }}>
...
function CircleProgress({ pct, size = 80, gradientId }: { pct: number; size?: number; gradientId: string }) {
  const stroke = 6;
  ...
  return (
    <div
      aria-label={`${pct}% overall progress`}
      role="img"
    ...
    >
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ position: "relative" }}>
        ...
        <circle
          ...
          stroke={`url(#ring-${gradientId})`} strokeWidth={stroke}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, paddingLeft: 4 }}>
        ...
        <div style={{ fontFamily: "'Fira Code',monospace", fontSize: 10.5, color: T.muted2, marginBottom: 8 }}>
          {lastTab ? `// last: ${lastTab}` : "// resume"}
        </div>
...
const TrackCard = memo(function TrackCard({
  ...
  return (
    <button
      onClick={onClick}
      aria-label={`${name} — ${done ? "completed" : `${pct}%`}`}
      style={{
        position: "relative",
        background: T.surface, border: `1px solid ${T.border}`,
    ...
    >
      ...
      <div style={{
        position: "relative", fontSize: 11, color: T.muted2,
        lineHeight: 1.45, minHeight: 30, marginBottom: 10,
      }}>{desc}</div>
      ...
      <div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <span style={{ fontSize: 10, color: T.muted, fontFamily: "'Fira Code',monospace" }}>{pct}%</span>
      </div>
    </button>
  );
});
