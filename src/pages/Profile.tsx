 ...
 *  2. displayName, memberSince, and recentPages all use storage service.
 *  3. memberSince is initialised once per user login and stored server-side
 *     in the future; for now it still uses localStorage but through the service.
 *  4. All expensive derivations memoised.
 *  5. AnimatedBar uses CSS instead of a rAF-based state update.
 *  6. "Recent activity" is limited to progress entries with a score —
 *     ordering is stable (not mutated in place).
 */
...
const COURSE_LESSONS: Record<string, { label: string; ids: string[]; color: string }> = {
  ...
  javascript: { label: "JavaScript", ids: ["js-basics","js-inter","js-adv"],                     color: T.amber  },
  desktop:    { label: "Desktop",    ids: ["tkinter","kivy"],                                    color: T.sky    },
  web:        { label: "Web",        ids: ["html-css"],                                          color: T.amber  },
  cpp:        { label: "C++",        ids: ["cpp-basics","cpp-inter","cpp-adv"],                  color: T.sky    },
  csharp:     { label: "C#",         ids: ["cs-basics","cs-inter","cs-adv"],                     color: T.rose   },
};

const pageLabel = (id: string) => ALL_ITEMS.find(i => i.id === id)?.label ?? id;

function getOrSetMemberSince(): string {
  const stored = storage.getMemberSince();
  if (stored) return new Date(stored).toLocaleDateString();
  const today = new Date().toISOString();
  storage.setMemberSince(today);
...
export default function Profile() {
  ...
  // recentPages kept for future use (e.g. "Recently visited" section).
  ...
  const memberSince  = getOrSetMemberSince();

  const skills = useMemo(() =>
    Object.entries(COURSE_LESSONS).map(([key, c]) => {
  ...
  const { accuracy, quizzes, minutes } = useMemo(() => {
    const scored = Object.values(progress)
      .map(p => p?.score)
      .filter((s): s is number => typeof s === "number");
    const acc  = scored.length
      ? Math.round(scored.reduce((a, b) => a + b, 0) / scored.length)
      : 0;
    const mins = Math.round(storage.getTimeSpent() / 60);
    return { accuracy: acc, quizzes: scored.length, minutes: mins };
  }, [progress]);
  ...
  const activity = useMemo(() =>
    Object.entries(progress)
      .filter(([, p]) => p?.score != null || p?.completed)
      .slice(0, 6)
      .map(([id, p]) => ({
        id,
        label: pageLabel(id),
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <ProfileCard>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div
              aria-label={`Profile avatar: ${displayName || user?.email || "Guest"}`}
              style={{
              ...
              {activity.map(a => (
                <button
                  ...
                  style={{
                    ...
                    color: T.text, textAlign: "left", transition: "all .15s",
                  }}
      ...
      {data.map(d => (
        <div key={d.name} style={{
          ...
          <div style={{ width: "100%", flex: 1, display: "flex", alignItems: "flex-end" }}>
            <ProgressBar pct={(d.pct / maxPct) * 100} color={d.color} vertical />
          </div>
          <div style={{
            fontSize: 10, color: T.muted2, fontFamily: "'Fira Code',monospace",
...
/**
 * ProgressBar — pure CSS animation (no rAF / useState).
 * Uses CSS @keyframes via a data attribute so the animation fires on mount.
 */
